...
const verifyToken = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (token.split(' ').length !== 2) return res.status(403).send({ auth: false, message: 'No token provided' })

    // Verifica se o token é válido e está no prazo de validade
    verify(token.split(' ')[1], process.env.SECRET as string, (err, decoded: any) => {
